...
final class GetPreviousWeekScheduleAction implements RequestHandlerInterface
{
    ...
    /**
     * @OA\Get(
     *     path="/todo/daily/previous-week/{id}",
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        $nextSchedule = $this->schedules->findPreviousWeekSchedule($person, $schedule);
        if (!$nextSchedule) {
            $this->validator->validate($command = new Command(
                $schedule->getDate()->modify('-7 day'),
                $userId
            ));
